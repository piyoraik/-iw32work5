"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteUser = exports.updateUser = exports.postUser = exports.fetchOne = exports.fetchAll = void 0;
var promise_1 = __importDefault(require("mysql2/promise"));
var config_1 = require("../config");
var TABLE = 't01_users';
var fetchAll = function () { return __awaiter(void 0, void 0, void 0, function () {
    var connection, sql, rows, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, promise_1.default.createConnection(config_1.dbsetting)];
            case 1:
                connection = _a.sent();
                sql = "SELECT * FROM " + TABLE;
                return [4 /*yield*/, connection.execute(sql)];
            case 2:
                rows = (_a.sent())[0];
                return [2 /*return*/, {
                        response: 200,
                        rows: rows,
                    }];
            case 3:
                err_1 = _a.sent();
                return [2 /*return*/, {
                        response: 400,
                        message: 'Error!',
                    }];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.fetchAll = fetchAll;
var fetchOne = function (params) { return __awaiter(void 0, void 0, void 0, function () {
    var connection, sql, rows, err_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, promise_1.default.createConnection(config_1.dbsetting)];
            case 1:
                connection = _a.sent();
                sql = "SELECT * FROM " + TABLE + " WHERE id = ? LIMIT 1";
                return [4 /*yield*/, connection.execute(sql, [params])];
            case 2:
                rows = (_a.sent())[0];
                if (Object.keys(rows).length === 0)
                    throw 'データが見つかりませんでした。';
                console.log(rows);
                return [2 /*return*/, {
                        response: 200,
                        rows: rows,
                    }];
            case 3:
                err_2 = _a.sent();
                return [2 /*return*/, {
                        response: 400,
                        message: 'Error!',
                    }];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.fetchOne = fetchOne;
var postUser = function (req) { return __awaiter(void 0, void 0, void 0, function () {
    var connection, sql, rows, err_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, promise_1.default.createConnection(config_1.dbsetting)];
            case 1:
                connection = _a.sent();
                sql = "INSERT INTO " + TABLE + " (id, username, email, password) VALUES (?, ?, ?, ?)";
                return [4 /*yield*/, connection.execute(sql, [
                        req.id,
                        req.username,
                        req.email,
                        req.password,
                    ])];
            case 2:
                rows = (_a.sent())[0];
                return [2 /*return*/, {
                        response: 200,
                        message: 'Success!!',
                    }];
            case 3:
                err_3 = _a.sent();
                return [2 /*return*/, {
                        response: 400,
                        message: 'Error!',
                    }];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.postUser = postUser;
var updateUser = function (req) { return __awaiter(void 0, void 0, void 0, function () {
    var connection, sql, rows, err_4;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, promise_1.default.createConnection(config_1.dbsetting)];
            case 1:
                connection = _a.sent();
                sql = "UPDATE " + TABLE + " SET username = ?, email = ?, password = ? WHERE id = ?";
                return [4 /*yield*/, connection.execute(sql, [
                        req.username,
                        req.email,
                        req.password,
                        req.id,
                    ])];
            case 2:
                rows = (_a.sent())[0];
                return [2 /*return*/, {
                        response: 200,
                        message: 'Success!!',
                    }];
            case 3:
                err_4 = _a.sent();
                console.log(err_4);
                return [2 /*return*/, {
                        response: 400,
                        message: 'Error!',
                    }];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.updateUser = updateUser;
var deleteUser = function (id) { return __awaiter(void 0, void 0, void 0, function () {
    var connection, sql, rows, err_5;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                return [4 /*yield*/, promise_1.default.createConnection(config_1.dbsetting)];
            case 1:
                connection = _a.sent();
                sql = "DELETE FROM " + TABLE + " WHERE id = ?";
                return [4 /*yield*/, connection.execute(sql, [id])];
            case 2:
                rows = (_a.sent())[0];
                return [2 /*return*/, {
                        response: 200,
                        message: 'Success!!',
                    }];
            case 3:
                err_5 = _a.sent();
                return [2 /*return*/, {
                        response: 400,
                        message: 'Error!',
                    }];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.deleteUser = deleteUser;
